...
# 配置文件路径
SUBTREE_CONFIG = SCRIPT_DIR / "subtree_repos.json"


def ensure_config_dir():
    """确保配置目录存在"""
    SUBTREE_CONFIG.mkdir(parents=True, exist_ok=True)
    if not SUBTREE_CONFIG.exists():
        with open(SUBTREE_CONFIG, 'w', encoding='utf-8') as f:
...
def load_subtree_repos() -> List[Dict[str, Any]]:
    """加载已保存的subtree仓库配置"""
    try:
        with open(SUBTREE_CONFIG, 'r', encoding='utf-8') as f:
            config = json.load(f)
...
def save_subtree_repo(repo_info: Dict[str, str]):
    """保存subtree仓库配置"""
...
def handle_working_tree_changes():
    """处理工作区未提交的更改"""
...
def add_subtree():
    ...
    if success:
        ...
        repo_info = {
            ...
            "prefix": prefix,
            "branch": branch,
            "added_time": datetime.now().isoformat(),  # 使用 datetime 获取当前时间
            "extra": {}  # 预留扩展空间
        }
...
def add_to_taskfile(prefix, remote, branch):
...
if __name__ == "__main__":
    try:
        sys.exit(0 if add_subtree() else 1)
